<?php 
    ...
    $author_url = 'http://127.0.0.1/quotes/api/authors/';
    $category_url= 'http://127.0.0.1/quotes/api/categories/';
    ...
    if ($action == 'read') {
        ...
        $quote_url = 'http://127.0.0.1/quotes/api/quotes/';
    ...
    elseif ($action == 'sort') {
        ...
        if (!empty($_GET['authorId']) && !empty($_GET['categoryId'])) {
            $quote_url = 'http://127.0.0.1/quotes/api/quotes/' . '?authorId=' . $_GET['authorId'] . '&categoryId=' . $_GET['categoryId'];
        } elseif (!empty($_GET['authorId']) && empty($_GET['categoryId'])) {
            $quote_url = 'http://127.0.0.1/quotes/api/quotes/' . '?authorId=' . $_GET['authorId'];
        } else if (empty($_GET['authorId']) && !empty($_GET['categoryId'])) {
            $quote_url = 'http://127.0.0.1/quotes/api/quotes/' . '?categoryId=' . $_GET['categoryId'];
        } else {
            $quote_url = 'http://127.0.0.1/quotes/api/quotes/';
        }
